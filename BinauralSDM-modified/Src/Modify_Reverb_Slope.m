...
for band = 1 : length(BRIR_data.FilterBank_f1)
    ...
    disp(size(BRIR_mod));
    disp(size(H_corrected));
%     BRIR_mod = BRIR_mod + H_corrected;
    % Ensure that BRIR_mod and H_corrected have the same dimensions
    max_length = max(size(H_corrected, 1), size(BRIR_mod, 1));
    % Pad both H_corrected and BRIR_mod with zeros
    H_corrected_padded = [H_corrected; zeros(max_length - size(H_corrected, 1), size(H_corrected, 2))];
    BRIR_mod_padded = [BRIR_mod; zeros(max_length - size(BRIR_mod, 1), size(BRIR_mod, 2))];
    BRIR_mod = BRIR_mod_padded + H_corrected_padded;
